...
export const api = {
  ...
  verifyPatientId: (patientId: string) =>
    request<{ exists: boolean; patientId: string; patient?: any; message: string }>(
      `/authorizations/patients/verify-patient/${encodeURIComponent(patientId)}`
    ),
